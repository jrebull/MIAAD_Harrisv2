...
Sized for inclusion at 0.74\\textwidth (LNCS textwidth = 347pt): the figure is
...
ax.set_ylabel(r"Hypervolume ($\times 10^{6}$)", fontsize=7.3, labelpad=2)
# blind-sampling reference: random-restart mean across the whole ladder
ax.axhline(rr_mean, color="0.35", ls="--", lw=0.7, zorder=1)
ax.text(9.42, rr_mean - 0.0008, "random-restart mean", ha="right", va="top",
        fontsize=6.0, color="0.30", style="italic")
...
# top tier: competent MO-HHO + the four permutation-native paradigms
yb = 0.3262
