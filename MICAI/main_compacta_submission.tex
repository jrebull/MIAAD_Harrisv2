...
The \emph{ladder} of methods shares the greedy decoder, objectives, hypervolume reference point, and $50\times500$ budget; the methods differ only in how they explore. Two operate on the continuous SPV random keys in $\mathbb{R}^{G}$:
\emph{(a)}~the MOHHO of Section~\ref{sec:mohho}; and
\emph{(b)}~NSGA-II~\cite{deb} with simulated binary crossover (SBX, $\eta_c=20$, $p_c=0.9$), polynomial mutation ($\eta_m=20$, $p_m=1/d$), binary tournament selection, and elitist $(\mu+\lambda)$ replacement.
A third, \emph{(c)}~random restart, draws all $25{,}000$ candidates uniformly at random, isolating the decoder's contribution.
...
\emph{Random restart}---blind sampling with no search---reaches a mean \HV\ of $310{,}214$: $5.7\,\%$ above the real-coded NSGA-II and $2.5\,\%$ above the classic real-coded MOHHO (the naive swarm wins only $10/30$ paired seeds). Giving NSGA-II the same external archive barely helps ($293{,}666$ vs.\ $293{,}367$)---no archiving artifact.
